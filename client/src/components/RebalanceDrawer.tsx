      ...
      <div className={`drawer ${show ? 'show' : ''}`}>
          ...
          {step === 'INPUT' && (
            <form onSubmit={handleCalculate}>
              <p className="text-muted mb-4">
                Enter the amount you wish to invest. We will calculate the optimal assets to buy to bring your portfolio back to its targets.
              </p>
            ...
            <div className="h-100 d-flex flex-column">
              ...
              {result.suggestions.length === 0 ? (
                 <div className="text-center py-5 my-auto">
                   ...
                   <p className="text-muted">Your portfolio already matches your targets. No trades are necessary right now.</p>
                 </div>
              ) : (
                <>
                  ...
                  <ul className="list-group shadow-sm rounded-3 overflow-hidden">
                    {result.suggestions.map((item, index) => (
                      <li key={item.assetId} className={`list-group-item p-3 d-flex justify-content-between align-items-center ${index % 2 === 0 ? 'bg-light' : 'bg-white'}`}>
                        <div>
                          <h5 className="fw-bold mb-1">{item.name}</h5>
                          <div className="small text-muted d-flex gap-3">
                            <span>Target: <strong>{item.targetPercentage}%</strong></span>
                            <span>Current: <strong>{item.currentPercentage}%</strong></span>
                          </div>
                        </div>
                        <div className="text-end">
                          ...
                          <small className="text-muted text-uppercase" style={{fontSize: '0.7rem'}}>to buy</small>
                        </div>
                      </li>
                    ))}
                  </ul>
        ...
        <div className="p-4 border-top bg-white mt-auto">
          {step === 'INPUT' ? (
            ...
            <div className="d-flex gap-3">
              <button className="btn btn-outline-secondary btn-lg flex-grow-1 py-3" onClick={() => setStep('INPUT')}>
                <i className="bi bi-arrow-left me-2"></i> Back
              </button>
              <button className="btn btn-success btn-lg flex-grow-1 py-3 fw-bold shadow-sm" onClick={onClose}>
